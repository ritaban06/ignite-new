              ...
              return (
                <div className="bg-blue-900 bg-opacity-30 border border-blue-600 rounded-lg p-3 mb-4">
                  <p className="text-blue-300 text-sm mb-2">
                    <strong>Parent Folder Metadata:</strong> (read-only)
                  </p>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-blue-200 mb-1">Departments</label>
                      <div className="grid grid-cols-2 gap-2 p-2 rounded bg-blue-950 border border-blue-800">
                        {DEPARTMENTS.map(dept => (
                          <div key={dept} className="flex items-center">
                            <input
                              type="checkbox"
                              disabled
                              checked={parentFolder.departments?.includes(dept) || false}
                              className="mr-2 h-4 w-4 rounded border-blue-500 text-blue-600"
                            />
                            <label className="text-blue-100">{dept}</label>
                          </div>
                        ))}
                      </div>
                    </div>
                    <div>
                      <label className="block text-blue-200 mb-1">Years</label>
                      <div className="grid grid-cols-2 gap-2 p-2 rounded bg-blue-950 border border-blue-800">
                        {YEARS.map(year => (
                          <div key={year} className="flex items-center">
                            <input
                              type="checkbox"
                              disabled
                              checked={parentFolder.years?.map(String).includes(year) || false}
                              className="mr-2 h-4 w-4 rounded border-blue-500 text-blue-600"
                            />
                            <label className="text-blue-100">Year {year}</label>
                          </div>
                        ))}
                      </div>
                    </div>
                    <div className="col-span-2">
                      <label className="block text-blue-200 mb-1">Semesters</label>
                      <div className="grid grid-cols-4 gap-2 p-2 rounded bg-blue-950 border border-blue-800">
                        {SEMESTERS.map(semester => (
                          <div key={semester} className="flex items-center">
                            <input
                              type="checkbox"
                              disabled
                              checked={parentFolder.semesters?.map(String).includes(semester) || false}
                              className="mr-2 h-4 w-4 rounded border-blue-500 text-blue-600"
                            />
                            <label className="text-blue-100">Sem {semester}</label>
                          </div>
                        ))}
                      </div>
                    </div>
                    <div>
                      ...
                      <textarea
                        value={parentFolder.description || ''}
                        disabled
                        className="w-full px-2 py-1 rounded bg-blue-950 border border-blue-800 text-blue-100"
                        rows={2}
                      />
                    </div>
                    <div>
                      ...
                      <input
                        type="text"
                        value={Array.isArray(parentFolder.tags) ? parentFolder.tags.join(', ') : ''}
                        disabled
                        className="w-full px-2 py-1 rounded bg-blue-950 border border-blue-800 text-blue-100"
                      />
                    </div>
                    <div className="col-span-2">
                      <label className="block text-blue-200 mb-1">Access Control Tags</label>
                      <div className="flex flex-wrap gap-1 p-2 rounded bg-blue-950 border border-blue-800">
                        {Array.isArray(parentFolder.accessControlTags) && parentFolder.accessControlTags.length > 0 ? (
                          parentFolder.accessControlTags.map(tagName => (
                            <span key={tagName} className="px-2 py-1 bg-blue-600 text-white text-xs rounded">
                              {tagName}
                            </span>
                          ))
                        ) : (
                          <span className="text-blue-300 text-xs">No access tags</span>
                        )}
                      </div>
                    </div>
                  </div>
